...
function createTaskItemElement(task) {
  ...
  li.addEventListener("click", function() {
    ...
    renderTaskList(document.querySelector("ol"));
  });
  return li;
}
...
function renderTaskList(ol) {
  ...
  for(let task of state.taskList) {
    ol.appendChild(createTaskItemElement(task));
  }
  renderInput();
}
...
function addNewTask() {
  ...
  state.taskList.push({id: state.taskList.length, description: taskInput.value, complete: false});
  taskInput.value = "";
  state.inputtedText = taskInput.value;
  renderTaskList(document.querySelector("ol"));
}
...
newTask.addEventListener("input", function() {
  state.inputtedText = newTask.value;
  renderInput();
});
...
function renderInput() {
  let newTask = document.querySelector("#task-input");
  let addTask = document.querySelector("#add-task");
  newTask.value = state.inputtedText;
